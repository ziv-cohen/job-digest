...
def test_title_senior_engineering_manager(scoring_cfg):
    ...
    assert _score_title(job, scoring_cfg) == 25
...
def test_location_plzen_onsite(scoring_cfg):
    job = make_job(location="Plzeň, CZ", is_remote=False)
    assert _score_location(job, scoring_cfg) == 12
...
def test_location_generic_remote(scoring_cfg):
    ...
    assert _score_location(job, scoring_cfg) == 10
...
def test_company_type_pretagged_product(scoring_cfg):
    ...
    assert _score_company_type(job, scoring_cfg) == 20
...
def test_company_type_outsourcing(scoring_cfg):
    ...
    assert _score_company_type(job, scoring_cfg) == 15
...
def test_company_type_consulting(scoring_cfg):
    ...
    assert _score_company_type(job, scoring_cfg) == 8
...
def test_seniority_senior_manager_pretagged(scoring_cfg):
    ...
    assert _score_seniority(job, scoring_cfg) == 12

def test_seniority_manager_pretagged(scoring_cfg):
    job = make_job(seniority="manager")
    assert _score_seniority(job, scoring_cfg) == 8
...
def test_bonus_salary_disclosed(scoring_cfg):
    job = make_job(salary_min=100000)
    job.has_growth_signals = False
    assert _score_bonus(job, scoring_cfg) == 2

def test_bonus_salary_text(scoring_cfg):
    job = make_job(salary_text="€120k–150k")
    job.has_growth_signals = False
    assert _score_bonus(job, scoring_cfg) == 2

def test_bonus_freelancer_keyword(scoring_cfg):
    job = make_job(description="Open to B2B contract")
    job.has_growth_signals = False
    assert _score_bonus(job, scoring_cfg) == 3
    assert job.job_type == "freelancer"

def test_bonus_growth_signals(scoring_cfg):
    job = make_job()
    job.has_growth_signals = True
    assert _score_bonus(job, scoring_cfg) == 3

def test_bonus_all_signals(scoring_cfg):
    job = make_job(description="Series B startup, contract OK", salary_min=120000)
    job.has_growth_signals = True
    assert _score_bonus(job, scoring_cfg) == 8  # 3 + 2 + 3

def test_bonus_none(scoring_cfg):
    job = make_job()
    job.has_growth_signals = False
    assert _score_bonus(job, scoring_cfg) == 0
